"""
    This program sends three messages to three different queues (one message per queue) on the 
    RabbitMQ server. Each message is one temperature reading from a 'smart smoker' system
    represented by smart-smoker.csv. Messages are sent every 30 seconds.
    ...
    try:
        ...
        with open(input_file, "r") as file:
            ...
            for row in reader:
                ...
                ch.basic_publish(
                    exchange="", routing_key=third_queue_name, body=third_mess_encode
                )
                # print a message to the console for the user
